...
```bash
git checkout main
git merge release/v1.2.6
git push origin main
```

### 6. Create and Push Git Tag
```bash
git tag -a v1.2.6 -m "Release version v1.2.6"
git push origin v1.2.6
```

Optional - Update Main V8 Pro with the latest package version:
```bash
composer require rconfighub/vector-server-pkg:v1.2.6
```
...
- Create new release using the v1.2.6 tag
