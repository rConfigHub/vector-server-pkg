...
```bash
git add .
git commit -m "Prepare release v1.2.2"
...
```bash
git tag -a v1.2.2 -m "Release version v1.2.2"
git push origin v1.2.2
```

Optional - Update Main V8 Pro with the latest package version:
```bash
composer require rconfighub/vector-server-pkg:v1.2.2
```
...
- Create new release using the v1.2.2 tag
